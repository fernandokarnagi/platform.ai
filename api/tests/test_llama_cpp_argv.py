...
def test_defaults_include_owned_flags_and_omit_optional():
    argv = LlamaCppEngine.build_argv(_node(), "/Users/x/models")
    assert argv[:6] == ["--models-dir", "/Users/x/models", "--host", "0.0.0.0", "--port", "8080"]
...
def test_forbidden_extra_flags():
    for extra in ["--port 9", "--host 1.2.3.4", "-m x.gguf", "--model x.gguf", "--models-dir /tmp"]:
        with pytest.raises(ForbiddenExtraFlagsError):
            LlamaCppEngine.build_argv(_node(serverParams={"extraFlags": extra}), "/m")


def test_resolve_binary_checks_opt_homebrew():
    command = LlamaCppEngine.resolve_binary_command()
    assert "/opt/homebrew/bin/llama-server" in command


def test_preview_uses_models_dir_not_placeholder():
    command = LlamaCppEngine.preview_command(_node())
