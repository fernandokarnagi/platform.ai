# Graph Report - platform.ai  (2026-08-16)

## Corpus Check
- 88 files · ~38,336 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 455 nodes · 783 edges · 52 communities (45 shown, 7 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 15 edges (avg confidence: 0.76)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8da46d79`
...
2. `run_command()` - 22 edges
3. `_require_node()` - 18 edges
4. `node_helper()` - 16 edges
5. `parse_object_id()` - 14 edges
6. `download_model()` - 13 edges
7. `start_engine()` - 12 edges
8. `get_status()` - 10 edges
9. `is_local_node()` - 9 edges
10. `apply_node_location()` - 9 edges

## Surprising Connections (you probably didn't know these)
- `FakeResult` --uses--> `LlamaCppEngine`  [INFERRED]
  api/tests/test_models_ops.py → api/engines/llama_cpp.py
- `test_is_local_host()` --calls--> `is_local_host()`  [EXTRACTED]
  api/tests/test_helpers.py → api/helpers.py
- `test_ssh()` --calls--> `is_local_node()`  [EXTRACTED]
  api/routes/nodes.py → api/helpers.py
- `run_command()` --calls--> `is_local_node()`  [EXTRACTED]
  api/services/ssh.py → api/helpers.py
...
Cohesion: 0.09
Nodes (49): apply_node_location(), cluster_helper(), download_helper(), is_local_host(), is_local_node(), _iso(), _last_openai_check(), models_cache_helper() (+41 more)

### Community 2 - "nodes.py"
Cohesion: 0.06
Nodes (42): LlamaCppEngine, DeleteModelIn, StartEngineIn, chat(), delete_model(), delete_node(), download_model(), engine_logs() (+34 more)
...
Cohesion: 0.11
Nodes (23): node_status(), engine_status(), is_running(), _exc_text(), run_command(), _run_local(), SshError, SshResult (+15 more)
...
Cohesion: 0.18
Nodes (12): _flag_value(), ForbiddenExtraFlagsError, default_server_params(), PreviewIn, preview_llama_cpp(), _node(), test_defaults_include_owned_flags_and_omit_optional(), test_extra_flags_appended_last() (+4 more)
...
## Knowledge Gaps
- **85 isolated node(s):** `Related`, ``clusters``, ``nodes``, `Related`, `Generated API map` (+80 more)
...
- **Why does `LlamaCppEngine` connect `nodes.py` to `test_llama_cpp_argv.py`, `test_helpers.py`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `run_command()` connect `test_ssh.py` to `test_helpers.py`, `nodes.py`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Why does `parse_object_id()` connect `test_helpers.py` to `nodes.py`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **What connects `Related`, ``clusters``, ``nodes`` to the rest of the system?**
